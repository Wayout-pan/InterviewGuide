...
- 内联函数不一定会被内联：inline只是给编译器一个建议，建议可以内联，实际是否将内联函数进行内联，取决于编译器觉得该内联函数是否足够简单。
- 内联函数的内部结构必须简单，不能具有while，switch等复杂的语句；更不能在内联函数中递归调用（因为内联函数本就是为了减小函数执行时栈空间的大小，如果在内联函数中使用递归，就违背初心了）
